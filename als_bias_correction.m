function [F,RMSE,M,U,MB,UB] = als_bias_correction(R,k,varargin)
...
%   UB:          bias vectors for users
    ...
    for i=1:maxiters
        [M,U,MB,UB] = iterate(M0,U0,R_c,lambda);
        M0 = M;
        U0 = U;
    end
    
    % prediction function
    F = @(movie, users, test_mu) ...
            bsxfun(@max, ...
                bsxfun(@min, ...
                    (M(movie,:)*U(:,users) + UB(users))+MB(movie)+mu+(test_mu-mu), ...
...
function [M,U,MB,UB] = iterate(M0,U0,R,lambda)
...
%   R:      sparse matrix R centered around its mean
    ...
    parfor i=1:m
        ...
        if isempty(ratings)          % nobody rated this movie
            continue;
        end
    ...
    parfor i=1:u
        [movies, ~, ratings] = find(R(:,i));
        if isempty(ratings)          % this user did not rate anything
            continue;
        end
        X = [ones(size(movies,1),1) M0(movies,:)];% relevant movies
        y = ratings;
        % regularized linear regression
        factor = (lambda*size(y,1)*eye(k+1) + X'*X)\(X'*y);
        UB(i) = factor(1);
        ...
        % sum_squares(i) = norm(ratings - X*factor - MB(movies)').^2;
    end
    
    M = M0;
    U = U0;
    % D = sum(sum_squares)+lambda*(norm(GammaM*M)^2+norm(U*GammaU)^2+norm(MB)^2+norm(UB)^2);
